...
// ════════════════════════════════════════════════════
// ODB 묵상 한 번에 가져오기 (wp-json content.rendered 직접 추출)
// ── 기존 findOdbUrlForDate→fetchOdbContent 2단계를 대체.
//    이유: ODB article 영구주소가 /년/일/월/ 구조라 web_fetch로 안 열려 404 →
//    "JSON 시작 문자 없음" 실패. wp-json은 글 본문(content.rendered)을 그대로 주므로
//    깨진 주소를 거치지 않고 원천에서 바로 추출한다. (콘솔 진단으로 검증됨)
// ════════════════════════════════════════════════════
async function fetchOdbDevotion(targetKoreaDate) {
  const odbDate = getOdbDateFor(targetKoreaDate);
  const targetYmd = fmtKey(odbDate);          // 예: 2026-06-22
  const range = odbRestRange(odbDate);        // ±2일 윈도우
  const restUrl = 'https://ourdailybreadministries.ca/wp-json/wp/v2/posts'
    + `?after=${range.after}&before=${range.before}`
    + '&per_page=30&orderby=date&order=desc&_fields=id,date,slug,link,title,content';
...
message_body must be complete with nothing omitted. No code fences.`;
  ...
  if (!meta.found || !meta.scripture_ref || !meta.message_body) return null;
  meta.message_body = (meta.message_body || '').replace(/"/g, "'").replace(/\n/g, ' ').trim();
...
export async function generateTodayDevotion(ctx) {
  ...
  try {
    // 1. ODB 글 + 본문 한 번에 (wp-json content 직접 추출 — 검증된 방식)
    setStep('0','active');
    const koreaDate = getKoreaDate();
    const devo = await fetchOdbDevotion(koreaDate);
    if (!devo) {
      const odbDate = getOdbDateFor(koreaDate);
      throw new Error(`${odbDateHuman(odbDate)}자 ODB 글을 가져오지 못했어요. 잠시 후 다시 시도해 주세요.`);
    }
